...
**Status:** Phase 1 implemented, plus continuous scroll pulled forward from Phase 2 as a user-toggled reading mode (single-page ↔ continuous, remembered per-browser via `localStorage`) rather than a full replacement of single-page mode — both modes now share one `PdfPage` component with real virtualization (IntersectionObserver-driven mount/unmount, not "render all N pages' canvases at once"). `tsc --noEmit`, `eslint . --max-warnings=0`, and `next build` all pass clean; **not yet visually verified in a browser** (the Claude-in-Chrome connector was unavailable all session). Split view, the in-reader AI panel, and dark-mode PDF rendering (the rest of Phase 3) remain undone. · **Routes touched (new):** `app/read/[paperId]` (outside the `(app)` route group — see §5) · **Companion to:** `01.FocusOS-v2-Plan.md` §8.6/§8.7 (Layered Notes, highlighting), `components/paper-annotation-viewer.tsx` (removed — superseded by this)
